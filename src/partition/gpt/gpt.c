...
// Генерация случайного GUID (версия 4, RFC 4122)
...
// Запись защитного MBR (первый сектор)
static ErrorCode write_protective_mbr(Disk *disk, uint64_t disk_sectors) {
    MbrSector mbr;
    memset(&mbr, 0, sizeof(mbr));
    // Одна запись раздела с типом 0xEE (GPT protective)
...
ErrorCode gpt_init(Disk *disk) {
    ...
    if (disk_sectors < GPT_MIN_SIZE) return ERR_DISK_CREATE;

    // Записываем защитный MBR
    ErrorCode err = write_protective_mbr(disk, disk_sectors);
    if (err != ERR_OK) return err;

    // Создаём заголовок GPT
    GptHeader header;
    ...
    // Выделяем память под таблицу разделов (пока пустую)
    ...
    // Резервная таблица (перед последним сектором)
    ...
    // Резервный заголовок (последний сектор)
...
ErrorCode gpt_create_partition(Disk *disk, int index, uint64_t size_sectors, const uint8_t *type_guid) {
    if (!disk || !disk->is_open) return ERR_DISK_OPEN;
    if (!type_guid) return ERR_INVALID_ARGUMENT;

    // Чтение заголовка
    GptHeader header;
    ErrorCode err = disk_read(disk, &header, sizeof(header), 1 * SECTOR_SIZE);
    if (err != ERR_OK) return err;
    if (memcmp(header.signature, GPT_SIGNATURE, 8) != 0) return ERR_INVALID_SIGNATURE;
...
void gpt_print_info(Disk *disk) {
    ...
    for (uint32_t i = 0; i < header.num_partition_entries; i++) {
        if (!is_partition_empty(&partitions[i])) {
            ...
            // Простейшее преобразование имени (ASCII)
    ...
    if (count == 0) printf("(no partitions)\n");
    free(partitions);
}

// Таблица соответствия имён и GUID
static const struct {
    const char *name;
    uint8_t guid[16];
} gpt_type_table[] = {
...
bool gpt_type_from_name(const char *name, uint8_t *guid) {
    if (!name || !guid) return false;
    char name_upper[32];
    if (!strlcpy_safe(name_upper, sizeof(name_upper), name)) return false;
...
int gpt_guid_from_string(const char *str, uint8_t *guid) {
    ...
    // Проверяем, что строка закончилась (опционально)
    while (*p == '-') p++;
    if (*p != '\0') return -1;
    return 0;
}